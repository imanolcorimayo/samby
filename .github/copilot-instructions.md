...
- **Components**:
  ...
  - `products/ProductDetails.vue` - Modal for viewing product details with seasonality
  - `products/ProductForm.vue` - Modal for creating/editing products
...
- `/dashboard`
  - `/dashboard/index.vue` - Main dashboard overview with seasonal metrics, spoilage alerts, and key fresh produce notifications

### 2. Productos (Fresh Produce Inventory)

- `/productos`
  - `/productos/index.vue` - Fresh produce inventory overview with modals for:
    - `products/ProductDetails.vue` - View product details with seasonality and freshness data
    - `products/ProductForm.vue` - Add/edit products with seasonal availability
...
- `/proveedores`
  - `/proveedores/index.vue` - Supplier directory with produce specialties and modals for:
...
- `/reportes`
  - `/reportes/index.vue` - Fresh produce analytics dashboard with:
...
- `/configuracion`
  - `/configuracion/index.vue` - General settings with tabs for:
    - Store settings with seasonal parameters
    - Delivery zones and fresh produce handling
    - System parameters and expiration alerts
  - `/configuracion/empleados.vue` - Employee management for produce businesses
...
- **Utils functions**: For commons functions like "formatCurrency", "formatQuantity", "formatSeason", or "calculateFreshness" let's always create the function in `@/utils/index.ts` if doesn't exist already.
